...
void Coin::paint(pro2::Window& window) const {
    ...
    //One of 8 sprites is chosen depending on the value of spin_vel_, which is always smaller than spin_counter;
    if (spin_counter_ > spin_vel_*(0.875))
        paint_sprite(window, topleft_, coin_texture_[0]);
    else if (spin_counter_ > spin_vel_*(0.75))
        paint_sprite(window, topleft_, coin_texture_[1]);
    else if (spin_counter_ > spin_vel_*(0.625))
        paint_sprite(window, topleft_, coin_texture_[2]);
    else if (spin_counter_ > spin_vel_*(0.5))
        paint_sprite(window, topleft_, coin_texture_[3]);
    else if (spin_counter_ > spin_vel_*(0.375))
        paint_sprite(window, topleft_, coin_texture_[4]);
...
const vector<Sprite> Coin::coin_texture_ = {
    {
